...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("aonecoin://", Qt::CaseInsensitive))
    {
       _request.replace(0, 13, "aonecoin:");
    }
    if(!_request.startsWith("aonecoin:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with aonecoin:"), QtCriticalMsg));
      return;
    }
 ...
 ConfirmSendDialog dlg(&MainWindow::instance());
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      if (m_ui->donateCheckBox->isChecked()) {
          CryptoNote::WalletLegacyTransfer walletTransfer;
          walletTransfer.address = "Xaone8xY8bNfWE2Ciuqidq9cKGNsHE4xacWA1u7788dmZRDCjtnwhiHYbFg8so6sk16V3uzvbicBiPrhteu9ZMVYentK6o98WLd";
