...
// Copyright (c) 2018 The Aonecoin developers
...
int main(int argc, char* argv[]) {
  ...
  QSettings karbowanecKey("HKEY_CLASSES_ROOT\\aonecoin", QSettings::NativeFormat);
  karbowanecKey.setValue(".", "Aone Wallet");
  karbowanecKey.setValue("URL Protocol", "");
  QSettings karbowanecOpenKey("HKEY_CLASSES_ROOT\\aonecoin\\shell\\open\\command", QSettings::NativeFormat);
  ...
  //as root
  args << "-c" << "printf '[Desktop Entry]\\nName = Aone URL Handler\\nGenericName = Aone\\nComment = Handle URL Sheme aonecoin://\\nExec = " + QCoreApplication::applicationFilePath() + " %%u\\nTerminal = false\\nType = Application\\nMimeType = x-scheme-handler/aonecoin;\\nIcon = Aone-Wallet' | tee /usr/share/applications/aonecoin-handler.desktop";
